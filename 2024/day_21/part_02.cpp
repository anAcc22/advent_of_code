...
int main() {
  ...
  auto solve = [&](const string &base) {
    ...
    auto solve = [&](this auto self, string s, int layer = 1) -> ll {
      if (layer == LAYERS) return sze(s);
      if (dp.contains({ s, layer })) return dp[{ s, layer }];
      ll ans = 0;
      for (int i : rng(sze(s))) {
        char u = i == 0 ? 'A' : s[i-1];
        set<string> cands = mp_side[{ u, s[i] }];
        ll mn = LONG_LONG_MAX;
        for (const auto &t : cands) {
          mn = min(mn, self(t+'A', layer+1));
        }
        ans += mn;
      }
      return dp[{ s, layer }] = ans;
    };
